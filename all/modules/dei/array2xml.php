...
function array2xml($array, $name='array', $standalone=TRUE, $beginning=TRUE) {
  ...
  $ArrayNumberPrefix = 'ARRAY_NUMBER_';
   ...
   foreach ($array as $root=>$child) {
    if (is_array($child)) {
      $output .= str_repeat(" ", (2 * $nested)) . '  <' . (is_string($root) ? str_replace(':','_',$root) : $ArrayNumberPrefix . str_replace(':','_',$root)) . '>';
      $nested++;
      ...
      $output .= str_repeat(" ", (2 * $nested)) . '  <' . (is_string($root) ? str_replace(':','_',$root) : $ArrayNumberPrefix . str_replace(':','_',$root)) . '><![CDATA[' . $child . ']]></' . (is_string($root) ? str_replace(':','_',$root) : $ArrayNumberPrefix . str_replace(':','_',$root)) . '>';
    }
  }
