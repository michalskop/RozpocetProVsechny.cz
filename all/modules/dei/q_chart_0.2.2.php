...
function q_common_chart($in) {
 ...
 if (isset($in['size'])) {
  ...
  if (isset($in['axis_tick_length'])) {
    $chart .= '&' . chxtc($in['axis_tick_length']);
  }
...
/**chxtc - AXES TICK LENGTH
* examples
* $axis_tick_length = array(0=> array(10), 1=> array(-180))
* 'chxtc=0,10|1,-180'
*/
function chxtc ($axis_tick_length) {
 //chxtc	axis tick length
  if (isset($axis_tick_length)) {
    foreach ($axis_tick_length as $key => $a_t_l) {
	  $chxtc .= $key . ',';
	  foreach ($a_t_l as $one_a_t_l) {
	    $chxtc .= $one_a_t_l . ',';
	  }
	  $chxtc = rtrim($chxtc,',');
	  $chxtc .= '|';
